...
class TestLaneCentering(OpenpilotTestCase):
  def test_params_defaults_are_registered(self):
    ctrl = LaneCenteringController()
  ...
  def test_hard_gates_pass_the_plan_through(self, kwargs):
    ctrl = controller()
  ...
  def test_lane_change_drops_a_built_up_correction(self):
  ...
  def test_steers_toward_lane_center(self):
  ...
  @parameterized.expand([2.5, 4.9])
  def test_implausible_lane_width_is_rejected(self, width):
  ...
  def test_confidence_loss_releases_smoothly(self):
  ...
  def test_blinker_fades_the_correction_out(self):
    md = model(left=-1.5, right=2.1)
  ...
  def test_blinker_pause_can_be_disabled(self):
    md = model(left=-1.5, right=2.1)
    ctrl, centered = converge(md)
    ctrl.pause_on_blinker = False
    assert correction(ctrl, md, car_state(left_blinker=True)) == approx(centered, atol=1e-7)

  def test_confident_model_keeps_authority_when_far_off_center(self):
  ...
  def test_unsure_model_does_not_keep_authority(self):
    _, out = converge(model(left=-1.0, right=2.6, path_std=0.6), authority=1.0)
    assert out > 0.0

  def test_model_authority_blends(self):
  ...
  def test_runs_on_a_real_model_message(self):
