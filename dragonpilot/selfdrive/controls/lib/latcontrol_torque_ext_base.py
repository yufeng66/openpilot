...
latcontrol_torque_ext_base.py, trimmed to the pieces the Lateral Jerk Torque
Controller needs (the NNFF/NNLC half of sunnypilot's extension is not ported).

Original lateral-jerk feedforward design by @twilsonco.

The torque-space helpers below live here rather than in opendbc because dp's
opendbc has no sunnypilot car extensions. Only GM overrides the torque-space
conversion upstream; every other brand uses the linear default reproduced here,
so keeping the callback shape means a future per-car override drops straight in.
"""
import math
...
def torque_from_lateral_accel_linear_in_torque_space(latcontrol_inputs: LatControlInputs,
                                                     torque_params: structs.CarParams.LateralTorqueTuning,
                                                     gravity_adjusted: bool) -> float:
  # The default is a linear relationship between torque and lateral acceleration
  # (accounting for road roll and steering friction)
  return latcontrol_inputs.lateral_acceleration / float(torque_params.latAccelFactor)


def get_friction_in_torque_space(lateral_accel_error: float, lateral_accel_deadzone: float, friction_threshold: float,
                                 torque_params: structs.CarParams.LateralTorqueTuning) -> float:
  """Torque-space twin of opendbc's get_friction. The stock helper multiplies by
  latAccelFactor because its caller sums into a lat-accel-space feedforward; here
  the feedforward is already torque, so friction is used as stored."""
...
class LatControlTorqueExtBase:
  def __init__(self, lac_torque, CP, CI):
    ...
    # dp: dp's opendbc has no per-car torque-space callback, so this is always
    # the linear default. Kept as an attribute to mirror sunnypilot's CI hook.
    self.torque_from_lateral_accel_in_torque_space = torque_from_lateral_accel_linear_in_torque_space
    self.torque_params = lac_torque.torque_params

    # Set by update() every frame from the host controller's state; the host's
    # PID object is shared, not a copy, so update_limits() below retargets it.
    self._ff = 0.0
    self._pid = None
    self._pid_log = None
    self._setpoint = 0.0
